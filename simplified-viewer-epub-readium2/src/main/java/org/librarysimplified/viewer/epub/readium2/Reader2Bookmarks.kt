...
object Reader2Bookmarks {
  ...
  fun loadBookmarks(
    ...
    val lastReadLocal =
      rawBookmarks.lastReadLocal?.let { toSR2Bookmark(it) }
    val lastReadServer =
      rawBookmarks.lastReadServer?.let { toSR2Bookmark(it) }
    val explicits =
      rawBookmarks.bookmarks.mapNotNull { toSR2Bookmark(it) }
  ...
  fun toSR2Bookmark(
    ...
    return when (val location = source.location) {
      is BookLocation.BookLocationR2 ->
        r2ToSR2Bookmark(source, location)
  ...
  private fun r2ToSR2Bookmark(
    ...
    // In Readium 3.x an SR2 locator's chapter href is a typed `Href`, constructed from the stored
    // string. If the stored href is not a valid URL, the bookmark cannot be represented in R2.
    val href = Href(location.progress.chapterHref) ?: return null
    return SR2Bookmark(
