...
import org.nypl.simplified.accounts.source.ekirjasto.AccountProviderResolutionErrorCodes.authDocumentParseFailed
import org.nypl.simplified.accounts.source.ekirjasto.AccountProviderResolutionErrorCodes.authDocumentUnusable
import org.nypl.simplified.accounts.source.ekirjasto.AccountProviderResolutionErrorCodes.authDocumentUnusableLink
import org.nypl.simplified.accounts.source.ekirjasto.AccountProviderResolutionErrorCodes.httpRequestFailed
import org.nypl.simplified.accounts.source.ekirjasto.AccountProviderResolutionErrorCodes.unexpectedException
...
/**
 * The logic needed to resolve a description into a full provider using standard E-kirjasto logic.
 */
...
class AccountProviderResolution(
  ...
  private fun extractAuthenticationDescription(
    ...
    if (authObjects.isNotEmpty()) {
      val ekirjastoAuthObject = authObjects.firstOrNull { authObject ->
        authObject is AccountProviderAuthenticationDescription.Ekirjasto
      }

      // basic token has the highest priority amongst the auth methods
      return if (ekirjastoAuthObject != null) {
        Pair(
          ekirjastoAuthObject,
          authObjects.minus(ekirjastoAuthObject)
