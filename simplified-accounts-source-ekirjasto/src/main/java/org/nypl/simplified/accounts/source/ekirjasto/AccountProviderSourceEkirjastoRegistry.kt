...
import org.nypl.simplified.accounts.source.ekirjasto.AccountProviderSourceEkirjastoRegistryException.*
...
class AccountProviderSourceEkirjastoRegistry(
  ...
  private val logger =
    LoggerFactory.getLogger(AccountProviderSourceEkirjastoRegistry::class.java)
  ...
  private var cacheFiles: CacheFiles? = null
  ...
  override fun load(
    ...
    this.cacheFiles = this.cacheFiles(context)
    val files = this.cacheFiles!!
  ...
  override fun clear(context: Context) {
    synchronized(this.writeLock) {
      this.cacheFiles = this.cacheFiles(context)
      FileUtilities.fileDelete(this.cacheFiles!!.file)
      FileUtilities.fileDelete(this.cacheFiles!!.fileTemp)
    }
  }

  override fun canResolve(description: AccountProviderDescription): Boolean {
    /*
     * We will actually resolve only the default/fallback account.
     */

    return description.id.toString().startsWith("defaultProvider:")
  }
  ...
  override fun resolve(
    ...
    var correctDescription = description
    if (description.id.toString().startsWith("defaultProvider:")) {
      if (this.cacheFiles == null) {
        throw CacheFilesDirNull()
      }

      val remoteDescriptions = this.fetchDiskResults(this.cacheFiles!!)
      val libraryId = URI(
        description.id.toString().replace("defaultProvider:", "urn:uuid:")
      )

      if (remoteDescriptions[libraryId] == null) {
        throw RemoteDescriptionForDefaultProviderNotFound()
      }
      correctDescription = remoteDescriptions[libraryId]!!.copy(id=description.id, isAutomatic = true)
    }
  ...
  private fun cacheFiles(context: Context): CacheFiles {
    return CacheFiles(
      file = File(context.filesDir, "org.nypl.simplified.accounts.source.ekirjasto.json"),
      fileTemp = File(context.filesDir, "org.nypl.simplified.accounts.source.ekirjasto.json.tmp")
    )
  }
        ...
        cacheFiles.fileTemp.outputStream().use { stream ->
          ...
          val collection =
            AccountProviderDescriptionCollection(
              ...
              links = listOf(Link.LinkBasic(
                href = uriProduction,
                type = MIMEType("application", "opds+json", mapOf()),
                relation = "self"
              )),
              metadata = meta
            )
  ...
  private fun decideRegistryURI(
    ...
    val debuggingBase =
      AccountProviderRegistryDebugging.properties[
        "org.nypl.simplified.accounts.source.ekirjasto.baseServerOverride"
      ]
    ...
    return this.parsers.createParser(target, stream, warningsAsErrors = false).use { parser ->
      when (val parseResult = parser.parse()) {
        ...
        is ParseResult.Failure -> {
          ...
          throw ServerReturnedUnparseableData(
            uri = target,
